...
function [positions_chickens, percentage_eating, dead, min_health, variance, moving_on, all_agent_health, alive_agent_health, deadness, eating, percentage_visited,  number_of_nodes, number_of_nodes_agents] = uninformed(graphing, dominance_hierachy, chickens, n, time, food_source, starting_chicken_health, food_amount)
...
number_of_nodes_agents = arrayfun(@(x) numel(unique(positions_chickens(x,:))), (1:size(positions_chickens,1)).');
    ...
    for agent = 1:chickens 
        if current_health(agent) > 1
            ...
            alive_agent_health(end+ 1) = 0;
    end 
    end 
    variance = var(alive_agent_health); 
